...
import { CoeffBadge } from './TierList';
...
interface PlayerRowProps {
  ...
  coeffMap?: Record<string, number | null>;
}
...
function adjustColor(hex: string, brightnessMul: number, saturationMul: number) {
  hex = hex.replace('#', '');
...
function getRankBackground(rank: number, points: number): string {
  ...
  if (points === 0 || rank > 300) return `linear-gradient(180deg, ${bgTop}, ${bgBottom})`;
  let baseColor = '';
  ...
  const tintedTop = mixColors(bgTop, adjustColor(baseColor, 1, 0.7), 0.1);
  const tintedBottom = mixColors(bgBottom, adjustColor(baseColor, 0.8, 0.45), 0.05);
  return `linear-gradient(180deg, ${tintedTop}, ${tintedBottom})`;
}

function getRankAccentColor(rank: number, points: number): string {
  if (points === 0 || rank > 300) return 'rgba(255,255,255,0.35)';
...
const rankBorderStyle = (rank: number, points: number): React.CSSProperties => ({
...
export default function PlayerRow({ player, rank, selectedMode, coeffMap }: PlayerRowProps) {
  ...
  return (
    ...
    >
      {/* Rank */}
      <div style={{
        width: 60, fontSize: 20, fontWeight: 700, letterSpacing: '0.3px',
        marginLeft: 20, color: rColor, textAlign: 'left', boxSizing: 'border-box',
      ...
      {/* Points */}
      <div style={{
        fontSize: 22, fontWeight: 800, letterSpacing: '0.3px', textAlign: 'center',
        color: scoreColor, textShadow: `0 0 2px ${scoreColor}`, fontVariantNumeric: 'tabular-nums',
      }}>
        {player._points}
      </div>
      ...
      {MODES.map(mode => {
        ...
        const coeff = coeffMap?.[mode.key] ?? null;
